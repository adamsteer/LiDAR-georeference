...
%%file names are set at the bottom of this file, in the 
% run command loop. No need to generate settings every time.

imufile = '../imu/mobile.rot'; %imu acceletation file
trajectoryfile = '20101109_bravo_utm45.3dp'; %trajectory file
...
r_adj = 0.0043; p_adj = 0.0035; y_adj = 0.0272;
...
files = dir('../lidar/*.rasc');


for i = 1:length(files)
    lidarfile = files(i).name;

    %hopefully print out a list of LIDAR files to process
    %lidarfile = file
    ...
    %all being well, batch process a whole flight...
    getdata; 
    makeswath;
end
